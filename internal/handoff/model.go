...
type Handoff struct {
	...
	// Integration state, generated by the orchestrator when a git_handoff's
	// commit is applied to the receiver's worktree. A sender never supplies
	// these, and "delivered" never implies "applied".
	IntegrationStatus string    // pending | integrated | failed | not-required
	IntegrationMethod string    // fast-forward | cherry-pick | none
	LocalCommit       string    // what that state is called in the receiving branch
	IntegratedAt      time.Time //
	IntegrationError  string    // why the last attempt failed
}

// Integration status values.
const (
	IntegrationPending   = "pending"
	IntegrationDone      = "integrated"
	IntegrationFailed    = "failed"
	IntegrationNotNeeded = "not-required"
)

// NeedsIntegration reports whether this handoff carries repository state that
// must be applied before the receiver can work on it.
func (h Handoff) NeedsIntegration() bool {
	return h.Type == TypeGit && h.CanonicalCommit != "" &&
